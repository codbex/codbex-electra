...
export class AttributeTranslationRepository {
    ...
    public count(Attribute: number): number {
        const resultSet = query.execute('SELECT COUNT(*) AS COUNT FROM "CODBEX_ATTRIBUTETRANSLATION" WHERE "ATTRIBUTETRANSLATION_ATTRIBUTE" = ?', [Attribute]);
