...
import * as dao from "../../dao/Products/AttributeGroupTranslation";
...
rs.service()
	...
	.resource("count/{AttributeGroup}")
		.get(function(ctx, request) {
			...
			http.sendResponseOk("" + dao.count(AttributeGroup));
		})
		.catch(function(ctx, error) {
            if (error.name === "ForbiddenError") {
                http.sendForbiddenRequest(error.message);
	...
	.resource("{id}")
		.get(function(ctx) {
			...
			if (entity) {
				...
				http.sendResponseNotFound("AttributeGroupTranslation not found");
			}
		})
		.produces(["application/json"])
		.catch(function(ctx, error) {
	...
	.resource("")
		.post(function(ctx, request, response) {
			...
			response.setHeader("Content-Location", "/services/js/codbex-electra/gen/api/AttributeGroupTranslation.js/" + entity.Id);
