...
import * as dao from "../../dao/Products/AttributeTranslation";
...
rs.service()
	...
	.resource("count/{Attribute}")
		.get(function(ctx, request) {
			...
			http.sendResponseOk("" + dao.count(Attribute));
		})
		.catch(function(ctx, error) {
            if (error.name === "ForbiddenError") {
                http.sendForbiddenRequest(error.message);
	...
	.resource("{id}")
		.get(function(ctx) {
			...
			if (entity) {
				...
				http.sendResponseNotFound("AttributeTranslation not found");
			}
		})
		.produces(["application/json"])
		.catch(function(ctx, error) {
	...
	.resource("")
		.post(function(ctx, request, response) {
			...
			response.setHeader("Content-Location", "/services/js/codbex-electra/gen/api/AttributeTranslation.js/" + entity.Id);
