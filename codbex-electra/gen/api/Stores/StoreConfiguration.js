...
import * as dao from "../../dao/Stores/StoreConfiguration";
...
rs.service()
	...
	.resource("count/{Store}")
		.get(function(ctx, request) {
			let Store = parseInt(ctx.pathParameters.Store);
			Store = isNaN(Store) ? ctx.pathParameters.Store : Store;
			http.sendResponseOk("" + dao.count(Store));
		})
		.catch(function(ctx, error) {
            if (error.name === "ForbiddenError") {
                http.sendForbiddenRequest(error.message);
	...
	.resource("{id}")
		.get(function(ctx) {
			...
			if (entity) {
				...
				http.sendResponseNotFound("StoreConfiguration not found");
			}
		})
		.produces(["application/json"])
		.catch(function(ctx, error) {
	...
	.resource("")
		.post(function(ctx, request, response) {
			...
			response.setHeader("Content-Location", "/services/js/codbex-electra/gen/api/StoreConfiguration.js/" + entity.Id);
