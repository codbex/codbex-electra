angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', '$http', 'messageHub', 'entityApi', 'Extensions', function ($scope, $http, messageHub, entityApi, Extensions) {
		...
		Extensions.get('dialogWindow', 'codbex-electra-custom-action').then(function (response) {
			$scope.pageActions = response.filter(e => e.perspective === "sales-orders" && e.view === "SalesOrderShipping" && (e.type === "page" || e.type === undefined));
			$scope.entityActions = response.filter(e => e.perspective === "sales-orders" && e.view === "SalesOrderShipping" && e.type === "entity");
		});
		...
		$scope.loadPage = function (pageNumber, filter) {
			...
			entityApi.count(filter).then(function (response) {
				...
				entityApi.search(filter).then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("SalesOrderShipping", `Unable to list/filter SalesOrderShipping: '${response.message}'`);
						return;
					}
		...
		$scope.openFilter = function (entity) {
			messageHub.showDialogWindow("SalesOrderShipping-filter", {
