angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', '$http', 'messageHub', 'entityApi', function ($scope, $http, messageHub, entityApi) {
		...
		$http.get("/services/js/resources-core/services/custom-actions.js?extensionPoint=codbex-electra-custom-action").then(function (response) {
			$scope.pageActions = response.data.filter(e => e.perspective === "Settings" && e.view === "EntityReference" && (e.type === "page" || e.type === undefined));
			$scope.entityActions = response.data.filter(e => e.perspective === "Settings" && e.view === "EntityReference" && e.type === "entity");
		});
		...
		$scope.loadPage = function (pageNumber) {
			...
			entityApi.count().then(function (response) {
				...
				entityApi.list(offset, limit).then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("EntityReference", `Unable to list EntityReference: '${response.message}'`);
						return;
					}
